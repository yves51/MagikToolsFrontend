  ...
  const UpdateDepartement = () => {
    ...
    const { departement } = location.state as { departement: Departement };

    const initialiserFormulaire = (departement: Departement) => {
      setLibelle(departement.name);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        const modifierDepartement: Departement = {
          id: departement.id,
          name: libelle,
        ...
        await departementService.update(departement.id, modifierDepartement); // ✅ Passer l'ID et les données
        toast.success("Departement modifié avec succès !");
        setTimeout(() => {
    ...
    useEffect(() => {
      if (departement) {
        initialiserFormulaire(departement);
      }
    }, [departement]);
    ...
    return (
      <>
        <div className="bg-white p-4 rounded shadow-sm">
          <h2 className="mb-4">Modification de Departement</h2>
