...
import { goToAjouterMarques, goToModifierMarques } from "../../../../shared/utils/navigation";
...
const ListeMarque = () => {
  ...
  const getMarqueById = async (id: number) => {
    try {
      let response = await marqueService.getById(id);
      if (response.success) {
        goToModifierMarques(navigate, response.results)
      } else {
  ...
  const loadMarques = async () => {
    ...
    const deleteMarque = async (id: number) => {
      try {
        setPending(true);
        const response = await marqueService.delete(id);
        if (response.data.success) {
          toast.success(response.data.message || "Suppression réussie.");
          setMarques((prev) => prev.filter((p) => p.id !== id));
        } else {
  ...
  const confirmAndDeleteMarque = async (id: number) => {
  ...
  if (loading) {
    return (
      <div className="text-center py-5">
        ...
        <p className="mt-3">Chargement des marque...</p>
      </div>
    );
  }
        ...
        <nav className="mb-3" aria-label="breadcrumb">
          <ol className="breadcrumb mb-0">
            <li className="breadcrumb-item">
              <Link to="/dashboard/liste-departements">Marques</Link>
            </li>
            <li className="breadcrumb-item active">Marques</li>
          </ol>
        </nav>
        <h2 className="text-bold text-body-emphasis mb-5">La liste des Marques</h2>
        <div
          id="members"
          ...
          <div className="row align-items-center justify-content-between g-3 mb-4">
            ...
            <div className="col-auto">
              <div className="d-flex align-items-center">
                ...
                <button onClick={() => goToAjouterMarques(navigate)} className="btn btn-primary">
              ...
              <table className="table table-sm fs-9 mb-0">
                ...
                <tbody className="list">
                  {filteredMarques.map((marque, index) => (
                    <tr 
                      key={marque.id}
                      onClick={() => handleRowClick(marque.id)}
                      className={
                        selectedRowId === marque.id ? "selected-row" : ""
                      }
                      >
                      <td className="align-middle ps-3">{index + 1}</td>
                      <td className="align-middle">{marque.name.toUpperCase()}</td>
                      <td className="align-middle">
                        {new Date(marque.createdAt).toLocaleString("fr-FR", {
                          day: "2-digit",
                          month: "2-digit",
                          year: "numeric",
                          hour: "2-digit",
                      ...
                      <td className="align-middle">{marque.auteurCreate?.prenoms}</td>
                      <td className="align-middle">
                        {new Date(marque.updatedAt).toLocaleString("fr-FR", {
                          day: "2-digit",
                          month: "2-digit",
                          year: "numeric",
                          hour: "2-digit",
                      ...
                      <td className="align-middle">{marque.auteurUpdate?.prenoms}</td>
                        ...
                        <div className="btn-reveal-trigger position-static">
                          ...
                          <div className="dropdown-menu dropdown-menu-end py-2">
                            ...
                            <button
                              ...
                              onClick={() => getMarqueById(marque.id)}
                            >
                              Modifier
                            </button>
                            <div className="dropdown-divider"></div>
                             <button
                              ...
                              onClick={() =>
                                confirmAndDeleteMarque(marque.id)
                              }
                            >
                              Supprimer
                            </button>
